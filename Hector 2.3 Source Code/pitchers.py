...
def get_weights_dir():
    if getattr(sys, 'frozen', False):
        return Path(sys.executable).parent  # Compiled EXE case
    else:
        return Path(__file__).parent        # Script case - same folder as pitchers.py & pitcher_weights.py
...
def import_weights_module(module_name):
    ...
    print(f"DEBUG: Trying to load {module_name} from: {module_path} (Exists? {module_path.exists()})")
...
section_weights = pitcher_weights.section_weights

def load_pitchers_data(filename="pitchers.html"):
    html_path = get_weights_dir() / filename
